# -*- coding: utf-8 -*-
"""
Created on Tue Apr 28 17:00:24 2020
...
def CalSabr(F,K,T,arg,alpha,beta):
    
    rho,vega = arg[0],arg[1]
...
def getTdAlpha(pre_close,OpDB,td,maturity_date,rf):
    df = c.deepcopy(OpDB[(OpDB['trade_date']==td)&(OpDB['maturity_date']==maturity_date)])
    ...
    df['dis'] = abs(df['exercise_price']-df['underlying_pre_close'])
    arg = list(df[(df['dis']==df['dis'].min())&(df['call_put']=='C')][['exercise_price','pre_close']].iloc[0])
    if len(arg)==0:
        return
    return BT.CalVIX(pre_close,arg[0],arg[1],rf,T,1)
...
df['code'] = df.apply(lambda x: getTdOp(x['pre_close'],OpDB,x['trade_date'],x['maturity_date'],x['rf'],x['alpha'],x['beta']),axis=1)
