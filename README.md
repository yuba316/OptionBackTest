...
![MA策略回测结果](https://github.com/yuba316/OptionBackTest/blob/master/%E5%9B%BE%E7%89%87/MA%E7%AD%96%E7%95%A5%E5%9B%9E%E6%B5%8B%E7%BB%93%E6%9E%9C.png)
...
按照我认为对的方式花了1.5天的时间写完了新的回测函数，又花了不到1天的时间把舍友说的交易波动率策略给复刻了出来。实际上效果并不好，而且这次我用在给回测函数debug上的时间明显要长一些（虽然第一个回测函数用了将近一星期的每个早上，而这个只用了1.5天……），只能说，想要回测函数写得简单且有普适性，策略的设计就要麻烦很多，而策略设计简单了，回测函数就要考虑更多的情况，二者是相辅相成的（天呐我在说啥啊……）。  
策略设计的本身有很多漏洞，也主要是因为我自己不熟悉SABR模型（第一次接触……），而且获取到的signalDf有很多缺失值，虽然是在未开仓期间才存在的缺失值，但是具体原因还是得仔细考查……接下来到这星期结束前就是把这个策略完善一下吧，这个初版只是为了测试一下回测函数到底能不能用而已……加油加油。  
  
![SABR策略回测结果](https://github.com/yuba316/OptionBackTest/blob/master/%E5%9B%BE%E7%89%87/SABR%E7%AD%96%E7%95%A5%E5%9B%9E%E6%B5%8B%E7%BB%93%E6%9E%9C.png)
  
统计量 | 数值
---- | ----
累计收益率% |	1.0762999999999419
年化收益率% |	0.21338919065723996
年化波动率% |	0.41759991468668217
年化下行波动率% |	0.3449600708421686
夏普比率 |	0.005109895456212966
索提诺比率 |	0.006185909868822849
最大回撤 |	703785.5000000002
最大回撤百分比 |	0.7037855000000003
卡玛比率 |	0.3032020276877541
胜率 |	0.4759576202118989
盈亏比 |	1.022953351970709
